...
def modularRefreshPlaneAndKey(start, end, timesRan):
    global DATA, LENGTH, TOTALCHANNELS
    i = start
    channel = 0
    vertCounter = 0
...
def animate(deltaFrame = 1, planeLength = 20, channels = 4, animationLength = 200): #totalFrames should be global length but how do I make that call in the function declaration
    global DATA, LENGTH, TOTALCHANNELS
    waves = cmds.polyPlane(w=planeLength - 1, h=2, sx=planeLength - 1, sy=channels) #H is width
    start = 0
    end = planeLength * deltaFrame
    pplane = 1
    frame = 1
    totalFrames = 0

    while end < animationLength:
        modularRefreshPlaneAndKey(start, end, frame)
        frame += deltaFrame
        start += 1
        end += 1
